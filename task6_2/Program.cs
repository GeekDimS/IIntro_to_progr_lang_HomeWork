...
// значения b1, k1, b2 и k2 задаются пользователем. Коэффициенты - только целые числа.
...
// соответствуют перечислению их в уравнениях.
{
    ...
    if (arr.Length != 4) 
    {
        Console.WriteLine("Неправильные коффициенты");
        return res;
    }
    ...
    res[0] = (arr[1] - arr[3]) / (double)(arr[2] - arr[0]); // Расчёт абсциссы с преобразованием типа
    res[1] = ((arr[1] * arr[2]) - (arr[3] * arr[0])) / (double)(arr[2] - arr[0]); // Расчёт ординаты 
    return res;
}
